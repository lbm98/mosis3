...
class CBDRunner(AtomicDEVS):
	...
	def __init__(self, name, cbd, initials=None, stopped=False, crossings=None, algo=CrossingDetection.ITP, **kwargs):
		...
		for inp in filter(lambda b: b.getBlockType() == "InputPortBlock", cbd.getBlocks()):
			self.inputs[inp.getBlockName()] = self.addInPort(inp.getBlockName())
		self.stop = self.addInPort("stop")

		self.outputs = {}
		for out in cbd.getSignals().keys():
...
def prepare_cbd(model, initials):
	...
	blocks. Additionally, a :class:`CBD.lib.std.ConstantBlock` and a
	:class:`CBD.lib.std.MinBlock` will be added in-between the Clock and the block
	that connects to the Clock over the :code:`h` input.

	Args:
		model (CBD.Core.CBD):    The CBD model to prepare.
